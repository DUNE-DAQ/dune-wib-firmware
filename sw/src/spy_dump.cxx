...
void femb_seg(uint32_t *seg,char *femb) {
    ...
    for (size_t i = 0; i < 40; i++) {
        size_t ch = u_to_ch[i];
        printf("%s U[%2ld] ADC[%2ld] CH[%2ld]: 0x%04X\n",femb,i,ch/16,ch%16,adc_vals[i]);
    }
    for (size_t i = 0; i < 40; i++) {
        size_t ch = v_to_ch[i];
        printf("%s V[%2ld] ADC[%2ld] CH[%2ld]: 0x%04X\n",femb,i,ch/16,ch%16,adc_vals[i+40]);
    }
    for (size_t i = 0; i < 48; i++) {
        size_t ch = x_to_ch[i];
        printf("%s X[%2ld] ADC[%2ld] CH[%2ld]: 0x%04X\n",femb,i,ch/16,ch%16,adc_vals[i+80]);
    }
}
...
int main(int argc, char **argv) {
    ...
    if (!bin_in.is_open()) {
        fprintf(stderr,"Could not open binary dump: %s\n",argv[1]);
        return 0;
    }
    ...
    if (length != 0x200000) {
        printf("This does not look like a 2MB spy buffer file! Dumping raw data anyway...\n");
        dump_frames((frame14*)fbuf,length/sizeof(frame14));
    } else {
        printf("Dump spy buffer 0:\n");
        dump_frames((frame14*)fbuf,0x100000/sizeof(frame14));
        printf("Dump spy buffer 1:\n");
